...
export default class FormPrimaryDetails extends Component {
	continue = e => {
		...
		if (!this.props.values.name || !this.props.values.fare || !this.props.values.busNumber) {
			return Swal.fire({
				type: 'error',
		...
		return (
			<Layout title="Add new bus (Primary details)">
				...
				<div className="form-group">
					...
					<select className="form-control" value={values.type} onChange={handleChange('type')}>
